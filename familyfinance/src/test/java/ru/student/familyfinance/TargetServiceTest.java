...
public class TargetServiceTest {
    ...
    public void addTargetTest() {
        Target target = new Target(1, new Person(), "Test Target", 100.0);
        doReturn(target).when(repository).save(target);
        assertThat(service.addTarget(target)).isTrue();
    ...
    public void editTargetTest() {
        Target target = new Target(1, new Person(), "Test Target", 100.0);
        doReturn(target).when(repository).save(target);
        assertThat(service.editTarget(target)).isTrue();
        Mockito.verify(repository,Mockito.times(1)).save(target);
    }
    ...
    public void getTargetsTest() {
        ...
        List<Target> list = List.of(new Target(1, person, "Test Target", 100.0),
                                    new Target(2, person, "Second Test Target", 200.0),
                                    new Target(3, person, "Test Target too", 300.0));
